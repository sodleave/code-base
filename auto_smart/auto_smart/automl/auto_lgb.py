...
class AutoLGB(AutoML):
    ...
    def ensemble_train(self,X,y,categories,config,len_test):
        ...
        X_train,y_train = autosample.downsampling(X,y,max_sample_num,seed)  #可复用的负采样包
        ...
        for i in range(1,rest_model_num+1):
            ...
            cur_columns = list(pd.Series(all_columns).sample(frac=0.85,replace=False,random_state=seed))#后面融合的时候进行列抽样
            ...
            lrs = list(lrs * rands) #随机的扰动learningrate
            
            cur_iteration = self.best_iteration
            cur_iteration = cur_iteration + np.random.randint(-30,40)  #随机的扰动iteration
    ...
    def param_compute(self,X,y,categories,config):
        feat_name = list(X.columns)
        colsample_bytree = self.colsample_bytree_opt(X.shape[1])  #根据特征数确定列抽样比例，特征多抽样比例低
        self.hyperparams['colsample_bytree'] = colsample_bytree
        
        max_sample_num = len(y)
        subsample = self.subsample_opt(autosample.downsampling_y(y,max_sample_num).shape[0])  #根据待采样样本数寻找比例
        ...
        gc.collect()
        params = self.params   #前面先小样本抽样进行尝试，后面使用全量样本测试时间
        ...
        for number_boost_round in [700,600,500,400,300,200,100,50]:
            ...
            if real_model_time > rest_time:
                ...
                self.num_boost_round = number_boost_round   #确定时间允许的最大迭代次数
                break
    ...
    def param_opt_new(self,X_train,y_train,X_valid,y_valid,categories):
        ...
        lr = self.lr_opt(train_data,valid_data,categories)  #通过几组cv集的测试结果选择最优学习率
        del train_data
        gc.collect()
        self.learning_rates = lr
